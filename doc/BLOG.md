...
Since in my chat app, each chat room will be independent of another, we can spawn a goroutine per room and treat it completely independently. I will still want to track each room so that a connecting client can choose to join a set of rooms, for this I will use a set of Room pointers. 
...
TODO, describe the process of writing the code 
...
- realtime view of rooms 
    - can be solved with a separate room a wrapper, that will publish to a room called "RoomList"
        - pubsub.NewRoom() will send a message to all subscribers that a room "x" has been added, and consequently update UI
        - pubsub.StopRoom()  will send a message to all subscribers that a room "x" has been removed
